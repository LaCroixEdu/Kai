...
- Preserve the exact JSON value through the OpenAI-compatible response, tool-loop history, persisted conversation, and next OpenRouter assistant message. Persistence must also retain the assistant `tool_calls` envelope and tool-result IDs needed to reattach the opaque state after reload.
...
4. Conversation JSON save/load retains `reasoning_details`, assistant tool calls, and tool-result IDs, while old JSON without the new fields still loads with `null`/empty defaults.
